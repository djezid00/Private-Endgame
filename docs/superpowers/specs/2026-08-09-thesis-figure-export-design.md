...
Figures are numbered **only across the three phases in scope**, in Theory.md order
(§12 → §13 → §14), starting at 6.1. The 400k validation figures are deliberately *not* allocated
slots.

> **Accepted risk:** the numbers are baked into filenames. Chapter 6 is being restructured in a
> parallel session, so if the section order changes, files must be renamed and captions re-edited
> by hand. This was chosen over an index-driven scheme for simplicity of insertion.

FESB requires `Slika 6.1.` (dot separators, caption **below** the image, centred). Filenames use a
dash — `Slika_6-1_…` — because a trailing dot in a filename is fragile on Windows. The caption text
...
│   ├── Slika_6-1_sparse_vs_shaped_5M.{png,svg}
│   ├── Slika_6-2_ppo_stopa_hvatanja.{png,svg}
│   ├── Slika_6-3_ppo_elo.{png,svg}
│   ├── Slika_6-4_sonda_isporuke.{png,svg}
│   ├── Slika_6-5_gama_sonde_zetva.{png,svg}
│   ├── Slika_6-6_gama_krivulje_ucenja.{png,svg}
│   ├── Slika_6-7_gama_osjetljivost.{png,svg}
│   └── Slika_6-8_gama_elo_9_pokreta.{png,svg}
...
| № | Content | Scalar tags |
|---|---|---|
| 6.1 | **Main figure.** Three stacked panels — `Self-play/ELO`, group reward, individual reward — sparse vs shaped, 3 seeds, min–max band. The individual-reward panel is the visual proof of farming: it stays high while group reward sits at −1. | `Self-play/ELO`, `Environment/Group Cumulative Reward`, `Environment/Cumulative Reward` |
| 6.2 | Catch rate for all four 2×2 cells; only POCA+shaped stays at ~0.01 | `Environment/Catch` |
| 6.3 | Chaser ELO, 2×2 | `Self-play/ELO` |
| 6.4 | Delivery-channel probe, three curves: group-only ~0.01 / group+individual ~0.12 / individual-only 0.98 | `Environment/Catch` |
| 6.5 | γ probes: catch below 1 % for every γ, with the harvest ladder (+122.8 / +50.8 / +4.5) on the same figure | `Environment/Catch`, `Environment/Cumulative Reward` |
| 6.6 | Learning curves per γ with min–max band — the γ=0.995 bimodality is visible *only* graphically | `Environment/Catch` |
| 6.7 | Sensitivity: final catch rate and ELO gap vs γ, per-seed points | `Environment/Catch`, `Self-play/ELO` |
| 6.8 | `Self-play/ELO` for all 9 sweep runs; `g0995_s1` is flat to ~4.3M | `Self-play/ELO` |

6.6 and 6.7 are re-emissions of existing `plot_gamma.py` output at the new size and naming. The
other six are new plotting code — notably 6.2–6.4, which exist today only as hand-taken
TensorBoard screenshots with no generating script.

### Rendering parameters

- Width 6.3 in ≈ 16 cm, matching the A4 text column under FESB margins; 6.1 is taller (3 panels).
...
├── plot_5m.py               NEW — 6.1
├── plot_ppo.py              NEW — 6.2–6.4
├── plot_gamma.py            EXTENDED — 6.5–6.8
...
2. All 8 figures render as PNG + SVG; the 6.1 individual-reward panel visibly separates from the
