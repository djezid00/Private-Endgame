...
## 6.1 Odgovor na istraživačko pitanje

Rezultati (poglavlje 5) daju izravan odgovor na istraživačko pitanje postavljeno u uvodu: agent
...
## 6.2 Tumačenje preokreta rezultata između kratkog i dugog horizonta

Preokret rangiranja između 400 000 i 5 000 000 koraka (§5.3, §5.4) objašnjava se prirodom
...
## 6.3 Implikacije na metodologiju validacije MARL sustava

Rezultat pri 400 000 koraka (§5.3) sam za sebe upućivao bi na suprotan zaključak: da gusto
...
sustava: kratkoročni rezultati mogu poslužiti za provjeru ispravnosti implementacije (poglavlja 3 i
4, §5.1), ali ne i za konačno rangiranje dizajnerskih odluka poput oblikovanja nagrade, osim ako se
ne potvrde na horizontu bliskom onome koji se stvarno koristi za konačne modele.

## 6.4 Ograničenja rada

Nekoliko ograničenja treba uzeti u obzir pri tumačenju rezultata. Prvo, rezultati na dugom
horizontu (§5.4) očitani su iz konzolnih zapisa treniranja po ponašanju; potpuna agregacija podataka
preko tri sjemena (srednja vrijednost i standardna devijacija za stopu hvatanja, duljinu epizode i
vrijeme do hvatanja, s pripadajućim grafovima) iz TensorBoard zapisa još je u izradi. Drugo,
kvalitativna potvrda ponašanja izravnim promatranjem istreniranih agenata u Unity uređivaču (§5.5)
planirana je, ali u trenutku pisanja još nije formalno provedena i zabilježena. Treće, eksperiment
je proveden isključivo unutar jednog, pojednostavljenog scenarija igre lovice s kinematički
identičnim agentima; generalizacija zaključka na scenarije s asimetričnom kinematikom ili većim
brojem agenata po timu nije ispitana. Četvrto, usporedba s neovisnim PPO učenjem [7] — koja bi
izravno opravdala odabir MA-POCA algoritma umjesto jednostavnijeg pristupa — nije provedena u
sklopu ovog rada. Peto, testirana je samo jedna vrijednost koeficijenta PBS oblikovanja
(`coef = 0,5`); ponašanje pri manjim koeficijentima ostaje neispitano.
